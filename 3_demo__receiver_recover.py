...
功能：載入遭受竄改的影像，提取 2LSB、執行逆 Arnold Transform 與解 XOR、
      透過 16 區塊多數決投票選出可信任特徵，進行 6 階段受損定位與 Decoder 影像融合自癒修復。
...
def init_session():
    if "received_image" not in st.session_state:
        ...
        tam = to_display_uint8(er.embedded).copy()
        # 預設做一點遮蔽以供預覽
        tam[20:60, 20:60] = 0
        st.session_state.received_image = tam
    if "recover_result" not in st.session_state:
...
with st.sidebar:
    ...
    st.markdown(
        ...
        * **影像尺寸**：128×128 (pixel)
        * **區塊數量**：4×4 (每塊 32×32)
        * **金鑰種子 (Seed)**：`{st.session_state.seed}`
        """
    )
...
# 1. 接收影像輸入
st.markdown("### 1. 接收與載入待檢測影像")
col_up, col_preview = st.columns([1.2, 1])

with col_up:
    up_file = st.file_uploader("📥 上傳待檢測之影像 (PNG / JPG)", type=["png", "jpg", "jpeg"])
    if up_file is not None:
        file_bytes = np.frombuffer(up_file.read(), np.uint8)
        img = cv2.imdecode(file_bytes, cv2.IMREAD_GRAYSCALE)
        st.session_state.received_image = load_gray_image_from_array_or_path(img)
        st.session_state.recover_result = None
        st.rerun()

with col_preview:
    if st.session_state.received_image is not None:
        r_disp = to_display_uint8(st.session_state.received_image)
        st.image(to_rgb(cv2.resize(r_disp, (240, 240), interpolation=cv2.INTER_NEAREST)), caption="接收端目前影像", width=240)

# 執行修復與解密核心
if st.session_state.received_image is not None and st.session_state.recover_result is None:
    model, device = cached_model()
    st.session_state.recover_result = recover_watermark(
        st.session_state.received_image,
        model,
        device,
...
# 2. 解密與特徵還原展示
st.markdown("### 2. 浮水印提取與解密過程 (2LSB → 逆 Arnold → 解 XOR)")
st.caption("點選左側 4×4 任一區塊，右側將展示該區塊 2LSB 特徵提取、空間逆置換與解密流程：")

if rr is not None and st.session_state.received_image is not None:
    display_tampered_img = to_display_uint8(st.session_state.received_image)
            ...
            <div style="flex: 0 0 260px; position: relative; user-select: none;">
                <div style="position: relative; width: 260px; height: 260px; border-radius: 8px; overflow: hidden; border: 2px solid #000; background: #000;">
                    ...
                    <div id="dec-grid-overlay" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: grid; grid-template-columns: repeat(4, 1fr); grid-template-rows: repeat(4, 1fr);">
                        {"".join([f'<div class="dec-grid-cell" data-id="{i}" style="border: 1px solid #000000; box-sizing: border-box; cursor: pointer;"></div>' for i in range(16)])}
                    </div>
                    <div id="dec-hover-box" style="position: absolute; border: 2px dashed #3b82f6; background: rgba(59, 130, 246, 0.25); pointer-events: none; display: none;"></div>
                    <div id="dec-selected-box" style="position: absolute; border: 3px solid #2563eb; box-shadow: 0 0 12px rgba(37, 99, 235, 0.85); pointer-events: none; display: none;"></div>
                </div>
    ...
    with col_f1:
        st.image(to_rgb(rr.tampered), caption="1. 接收影像 (竄改圖)", use_container_width=True)
    with col_op1:
    ...
    with col_f2:
        st.image(to_rgb(green_img), caption="2. 保留正常區域", use_container_width=True)
    with col_op2:
        st.markdown("<div style='text-align: center; font-size: 22px; font-weight: bold; color: #475569; padding-top: 55px;'>+</div>", unsafe_allow_html=True)
    with col_f3:
        st.image(to_rgb(red_img), caption="3. 擷取重建修復區域", use_container_width=True)
    with col_op3:
        st.markdown("<div style='text-align: center; font-size: 22px; font-weight: bold; color: #475569; padding-top: 55px;'>➔</div>", unsafe_allow_html=True)
    with col_f4:
    ...
    # 6. 下載修復後影像
    _, rec_buf = cv2.imencode(".png", to_display_uint8(rr.recovered))
    st.download_button(
        label="💾 下載最終修復影像 (recovered_image.png)",
        data=rec_buf.tobytes(),
